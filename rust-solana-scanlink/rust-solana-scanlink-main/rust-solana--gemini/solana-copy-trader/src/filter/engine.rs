...
struct CandidateTrace {
    ...
    gate2_remote_started_at_ms: Option<u64>,
    gate2_result_ready_at_ms: Option<u64>,
    gate2_result_applied_at_ms: Option<u64>,
...
pub async fn run(
    ...
    let (creator_gate_result_tx, mut creator_gate_result_rx) =
        mpsc::channel::<CreatorGateResolved>(SCANNER_NEW_TOKEN_CHANNEL_CAPACITY);
    ...
    let mut creator_gate_stream_open = true;
    let mut buy_stream_open = true;
    let mut internal_stream_open = true;

    loop {
        if !gate1_stream_open
            && !creator_gate_stream_open
            && !buy_stream_open
        ...
        tokio::select! {
            ...
            maybe_creator_gate = creator_gate_result_rx.recv(), if creator_gate_stream_open => {
                let Some(msg) = maybe_creator_gate else {
                    creator_gate_stream_open = false;
                    continue;
                };
...
async fn handle_gate1_result(
    ...
    let gate1_result_applied_at_ms = now_ms();
    let Gate1Result {
        token,
        decision,
        gate1_at_ms,
    ...
    let gate2_task_spawned_at_ms = now_ms();
    candidates.insert(
        token.mint.clone(),
        Candidate {
            token: token.clone(),
            ...
            trace: CandidateTrace {
                gate1_at_ms: Some(gate1_at_ms),
                gate1_result_applied_at_ms: Some(gate1_result_applied_at_ms),
                gate2_task_spawned_at_ms: Some(gate2_task_spawned_at_ms),
                ..Default::default()
            },
        },
    );
    ...
    tokio::spawn(async move {
        let result = creator_gate(&shared_clone, &token_clone, gate2_task_spawned_at_ms)
            .await
        ...
        let _ = creator_gate_result_tx
            .send(CreatorGateResolved {
                mint: token_clone.mint.clone(),
                token: token_clone,
                result,
            })
            .await;
...
async fn creator_gate(
    ...
    if let Some(profile) = cached.clone() {
        let cache_lookup_done_at_ms = now_ms();
        ...
        .await;
        result.refresh_launch_returned_at_ms = Some(now_ms());
    ...
    if retry_after_ms > 0 {
        return Ok(creator_gate_provisional_pass(
            ...
            task_spawned_at_ms,
            Some(creator_gate_entered_at_ms),
            Some(cache_lookup_done_at_ms),
            None,
            None,
        ));
    }
    ...
    let refresh_launch = maybe_spawn_creator_profile_refresh(
        shared,
        token.clone(),
        api_key.to_string(),
        None,
    ...
    .await?;
    let refresh_launch_returned_at_ms = now_ms();
    let (remote_started_at_ms, response_rx) = match refresh_launch {
        ...
        CreatorRefreshLaunch::Inflight => {
            return Ok(creator_gate_provisional_pass(
                ...
                Some(creator_gate_entered_at_ms),
                Some(cache_lookup_done_at_ms),
                Some(refresh_launch_returned_at_ms),
                None,
            ));
        }
...
fn creator_gate_timeout_fallback(
    ...
    if let Some(profile) = cached {
        ...
        result.task_spawned_at_ms = task_spawned_at_ms;
        result.creator_gate_entered_at_ms = creator_gate_entered_at_ms;
        result.cache_lookup_done_at_ms = cache_lookup_done_at_ms;
        result.refresh_launch_returned_at_ms = refresh_launch_returned_at_ms;
        result.remote_started_at_ms = remote_started_at_ms;
        result.result_ready_at_ms = now_ms();
        return result;
    }
    ...
    let mut result = CreatorGateResult {
        passed: true,
        reason: if sniper_fast_timeout {
            format!(
                "gate2 provisional pass: sniper timeout {}ms | gate2_penalty={} | gate2_warnings={}",
                timeout_ms,
                GATE2_FAST_TIMEOUT_PENALTY_SCORE,
                "timeout_provisional_pass"
            )
        } else {
            format!("gate2 soft-pass: timeout {}ms", timeout_ms)
        },
        profile: None,
        penalty_score: if sniper_fast_timeout {
            GATE2_FAST_TIMEOUT_PENALTY_SCORE
        } else {
            0
        },
...
fn sniper_single_buyer_reject_reason(
    config: &AppConfig,
    candidate: &Candidate,
    stats: &WindowStats,
    path: Gate3Path,
    ...
    if !config.sniper_single_buyer_filter_enabled || path != Gate3Path::Fast {
        return None;
    }
    if stats.unique_sm_wallets.len() > 1 || stats.eligible_buyers > 1 {
        return None;
    }
...
async fn score_candidate(shared: &SharedState, mut candidate: Candidate) -> ScoreDecision {
    ...
    if let Some(filter_reason) = sniper_single_buyer_reject_reason(
        shared.config.as_ref(),
        &candidate,
        &stats,
        trigger.path,
        ...
        return ScoreDecision {
            ...
            reason: format!("{} | {}", filter_reason, reason),
            signal: None,
            mode: smart_money_mode_label(stats.mode).to_string(),
            path: "sniper_single_buyer_filter".to_string(),
            matched_buyers: stats.unique_sm_wallets.len(),
            early_buy_count: stats.buy_count,
...
async fn record_token_outcome(
    ...
    let timing = FilterTimingRecord {
        ...
        gate2_remote_started_at_ms: trace.gate2_remote_started_at_ms,
        gate2_result_ready_at_ms: trace.gate2_result_ready_at_ms,
        gate2_result_applied_at_ms: trace.gate2_result_applied_at_ms,
    ...
    if let Err(err) = append_jsonl(
        ...
        &json!({
            ...
            "gate2_remote_started_at_ms": timing.gate2_remote_started_at_ms,
            "gate2_result_ready_at_ms": timing.gate2_result_ready_at_ms,
            "gate2_result_applied_at_ms": timing.gate2_result_applied_at_ms,
...
mod tests {
    ...
    fn base_config() -> AppConfig {
        ...
        AppConfig {
            ...
            gate3_max_single_buyer_share: 0.55,
    ...
    #[test]
    fn sniper_single_buyer_filter_rejects_low_signal_microcap_noise() {
        let cfg = base_config();
        let candidate = Candidate {
            token: NewToken {
                mint: "mint".to_string(),
        ...
        let stats = WindowStats {
            ...
            sm_sol_total: 0.24,
            total_eligible_sol: 0.24,
            fastest_sm_ms: Some(4),
            fast_reached_at_ms: Some(4),
            soft_reached_at_ms: None,
            buy_count: 1,
            eligible_buyers: 1,
            unique_funders: 0,
        ...
        let scoring = ScoringContext {
            ...
            dynamic_narrative_bonus: 0,
            narrative: NarrativeAdjustment::default(),
            gate2_penalty_score: 0,
            gate2_warning_tags: Vec::new(),
        ...
        let reason =
            sniper_single_buyer_reject_reason(&cfg, &candidate, &stats, Gate3Path::Fast, &scoring)
                .expect("single buyer noise should reject");
        assert!(reason.contains("microcap noise"));
    }

    #[test]
    fn sniper_single_buyer_filter_allows_stronger_single_buyer_signal() {
        let cfg = base_config();
        let candidate = Candidate {
            token: NewToken {
                mint: "mint".to_string(),
            ...
            status: CandidateStatus::Active,
            gate1_risk: Gate1RiskProfile::default(),
            narrative_keywords: vec!["trump".to_string()],
            dynamic_narrative_keywords: Vec::new(),
            narrative_keyword_matches: Vec::new(),
            gate2_penalty_score: 0,
            gate2_warning_tags: Vec::new(),
        ...
        let stats = WindowStats {
            ...
            unique_sm_wallets: ["buyer".to_string()].into_iter().collect(),
            sm_sol_total: 0.42,
            total_eligible_sol: 0.42,
            fastest_sm_ms: Some(9),
            fast_reached_at_ms: Some(9),
            soft_reached_at_ms: None,
            buy_count: 1,
            eligible_buyers: 1,
            unique_funders: 0,
